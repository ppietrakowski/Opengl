...
MaterialParam::MaterialParam(const char* uniformName, std::shared_ptr<Texture> value, std::uint32_t texture_unit) :
    m_Parameter{TextureParameter{uniformName, value, texture_unit}}
{
...
{
    return std::any_cast<std::int32_t>(m_Getter(m_Parameter)->GetValue());
}
