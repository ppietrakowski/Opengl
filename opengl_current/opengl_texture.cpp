...
OpenGlTexture2D::OpenGlTexture2D(const ImageRgba& image) :
    OpenGlTexture2D{image.GetRawImageData(), TextureSpecification{image.GetWidth(), image.GetHeight(), TextureFormat::Rgba}}
{
}
...
void OpenGlTexture2D::Bind(std::uint32_t textureUnit) const
{
    glBindTextureUnit(textureUnit, m_RendererId);
}

void OpenGlTexture2D::Unbind(std::uint32_t textureUnit)
{
