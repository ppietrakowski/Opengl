...
public:
    static std::shared_ptr<IShader> GetShader(const std::string& filePath);
    static std::shared_ptr<ITexture2D> GetTexture2D(const std::string& filePath);
    static void AddTexture2D(const std::string& path, const std::shared_ptr<ITexture2D>& texture);
